...
DMS = NewType('DMS', (int, int, float))
LatLong = NewType('LatLong', (DMS, DMS))
...
def angle(point, transmitter):
    return 0

def curvature(lat: DMS):
...
def DMStoMetric(dmsVal):
    metric = dmsVal[0]*30.87*60*60 + dmsVal[1]*30.87*60 + dmsVal[2]*30.87
    return metric

def DecimalToMetric(decVal):
    dms = decimalDegreeToDMS(decVal)
    metric = DMStoMetric(dms)
    return metric

def MetricToDecimal(metric):
    seconds = metric/30.87
    degrees = int(seconds/3600)
    minutes = int(seconds/60)-degrees*60
    seconds = seconds - degrees*3600 - minutes*60
    dms = [degrees,minutes,seconds]
    dec = DMStoDecimal(dms)
    return dec

def MetricToDMS(metric):
    seconds = metric / 30.87
    degrees = int(seconds / 3600)
    minutes = int(seconds / 60) - degrees * 60
    seconds = seconds - degrees * 3600 - minutes * 60
